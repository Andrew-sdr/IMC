...
class IMC(var nome: String?, var peso: Float, var altura: Float, var imc: Float, var arredondar: Float) :Parcelable {
    ...
    constructor(name: String, peso: Float, altura: Float) : this(name,peso,altura,0.0f,0.00f)

    fun calcular() : String{

        val alt =altura/100
        val calc = peso / (alt*alt)
        val arredondar = String.format("%.2f", calc).toFloat()
        val msg =when (calc){
